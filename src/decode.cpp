...
int ffmpeg_video_decode(const std::string &addr, const int type,
	...
	av_log(NULL, AV_LOG_INFO,
			"stream path:%s, \nuse_hw_decode:%s, \nonly_key_frame:%s,\n",
			addr.c_str(), use_hw_decode ? "true" : "false",
	...
	//开始对视频流解码	
	err = ffmpeg_decode(ic, type, video_stream_id, cpu_cb, gpu_cb,
			use_hw_decode, only_key_frame,gpu_id,quality,interval);
	if (err != 0) {
...
int ffmpeg_decode(AVFormatContext *ic, const int type, const int stream_id,
	...
	if (use_hw_decode) {
		//Nvidia硬解码, AV_CODEC_ID_H264 cuvid.c     hevc_cuvid  h264_cuvid
//		codec = avcodec_find_decoder_by_name("hevc_cuvid");
	...
	if (NULL == codec) {
		av_log(NULL, AV_LOG_FATAL,
				"avcodec_find_decoder_by_name failed\n");
		return -1;
	}
	...
	av_opt_set(codec_ctx->priv_data, "gpu",gpu_id.c_str(),  0);
	//time_base 不需要赋值
...
//	codec_ctx->codec_id = codecpar->codec_id;
	...
	int buffer_size = av_image_get_buffer_size(format, codec_ctx->width,
			codec_ctx->height, align);
	unsigned char *buffer = (unsigned char*) av_malloc(buffer_size);
	if (NULL == buffer) {
		av_log(NULL, AV_LOG_FATAL, "allocate buffer failed!\n");
		return -4;
	}
	av_image_fill_arrays(frame_bgr->data, frame_bgr->linesize, buffer, format,
	...
	sws_ctx = sws_getContext(codec_ctx->width, codec_ctx->height,
			codec_ctx->pix_fmt, dest_width, dest_height, format,
			SWS_FAST_BILINEAR,
			NULL, NULL, NULL);
	...
	db->capacity = v;
	db->size = v;
	db->mem_type = MEM_CUDA;
	//DataBuffer *db2 = new DataBuffer();
	//db->data = image2;
	//db->capacity = v;
	//db->size = v;
	//db->mem_type = MEM_HOST;
	while (av_read_frame(ic, &pkt) >= 0) {
		...
		if (use_hw_decode) {
			 ...
			 * */
			cv::Mat image(frame->height, frame->width, cv_format,
